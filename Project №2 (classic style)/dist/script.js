...
var accordion = function accordion(triggersSelector) {
  var btns = document.querySelectorAll(triggersSelector); //const blocks = document.querySelectorAll(itemsSelector);
  ...
  btns.forEach(function (btn) {
    btn.addEventListener("click", function () {
      ...
      this.classList.toggle("active-style"); //Стилизация соседнего элемента

      this.nextElementSibling.classList.toggle("active-content"); //Проверяем, есть ли активный класс на элементе, по которому кликнули
...
var burger = function burger(menuSelector, burgerSelector) {
  var menuElem = document.querySelector(menuSelector),
      burgerElem = document.querySelector(burgerSelector); //Скрываем меню
...
var calc = function calc(size, material, options, promocode, result) {
  ...
  var calcFunction = function calcFunction() {
    ...
    if (sizeBlock.value == "" || materialBlock.value == "") {
      //Если юзер не выбрал один из элементов
      resultBlock.textContent = "Пожалуйста, выберите размер и материал картины";
    } else if (promocodeBlock.value == "IWANTPOPART") {
      //Если промокод верный
      resultBlock.textContent = Math.round(sum * 0.7);
    } else {
      //Если юзер заполнил данные, но не ввел промокод
...
var drop = function drop() {
  ...
  var singleInput = document.querySelector(".single-input");
  var designer = "assets/server.php";
  ...
  function preventDefaults(event) {
    ...
    event.preventDefault(); //Отмена всплытия
  ...
  fileInputs.forEach(function (input) {
    input.addEventListener("drop", function (event) {
      ...
      input.previousElementSibling.textContent = name; // if (event.path[0].classList.contains("single-input")) {
      // 	console.log("ok");
      // }
    });
  });
...
var filter = function filter() {
  var menu = document.querySelector('.portfolio-menu'),
      ...
      no = document.querySelector('.portfolio-no'); //Фильтруем контент по типу
  ...
  var typeFilter = function typeFilter(markType) {
    ...
    no.style.display = "none"; //Так же скрываем информационный блок

    no.classList.remove("animated", "fadeIn"); //Показываем нужный тип контента
    ...
    if (markType) {
      ...
      //Если фильтр не передан, показываем информационный блок
...
var forms = function forms() {
  ...
  var form = document.querySelectorAll("form"); //Получаем все инпуты

  var inputs = document.querySelectorAll("input");
  var windows = document.querySelectorAll("[data-modal]"); //Все модальные окна
  //Блоки форм, куда загружаются изображения

  var upload = document.querySelectorAll("[name='upload']"); //checkNumInputs("input[name='user_phone']");//Фильтр ввода (только цифры)
  ...
  upload.forEach(function (item) {
    item.addEventListener("input", function () {
      ...
      var dots; //Сколько точек поставить
      //Массив с 2 элементами (название файла до точки и расширение)

      var arr = item.files[0].name.split("."); //Смотрим сколько символов в имени и если > 6, то обрезаем и добавляем ...
  ...
  form.forEach(function (item) {
    item.addEventListener("submit", function (event) {
      event.preventDefault(); //Создаем блок для вывода сообщения

      var statusMessage = document.createElement("div");
      statusMessage.classList.add("status"); //Добавляем сообщение не к самой форме, а к ее род.элементу
      ...
      setTimeout(function () {
        //Теперь полностью скрываем форму
        item.style.display = "none";
      }, 400);
      var statusImg = document.createElement("img");
      statusImg.setAttribute("src", message.spinner);
      statusImg.classList.add("animated", "fadeInUp"); //Помещаем картинку в блок со статусом запроса

      statusMessage.appendChild(statusImg);
      var textMessage = document.createElement("div");
      textMessage.innerHTML = message.loading;
      statusMessage.appendChild(textMessage); //Вытаскиваем данные из формы

      var formData = new FormData(item); //Опционально можно перевести в формат JSON

      var api; //Куда отправлять форму
      ...
      console.log(api); // if (item.getAttribute('data-calc') === "calculation") {
      ...
      Object(_services_requests__WEBPACK_IMPORTED_MODULE_6__["postData"])(api, formData).then(function (res) {
        ...
        //Очищаем инпуты
        clearInputs(); //Удаляем блок с сообщением
...
var mask = function mask(selector) {
  ...
  var inputs = document.querySelectorAll(selector); //Перебираем инпуты и применяем к ним  маску
...
var modals = function modals() {
  ...
  var btnPressed = false; //функция привязки модального окна к определенному селектору
  ...
  function bindModal(triggerSelector, modalSelector, closeSelector) {
    ...
    trigger.forEach(function (item) {
      item.addEventListener("click", function (event) {
        ...
        modal.style.display = "block"; //Запрещаем скролл страницы при открытом модальном окне

        document.body.style.overflow = "hidden";
        /*При открытии модалки будет добавлен margin-right, чтобы
        убрать перемещение страницы в бок при отключении скролла*/

        document.body.style.marginRight = "".concat(scroll, "px"); //document.body.classList.add("modal-open");//Класс бутстрапа
      });
    }); //Закрытие модалки
    ...
    close.addEventListener("click", function () {
      ...
      modal.style.display = "none"; //Возвращаем скролл в дефолтное значение
  ...
  function showModalByTime(selector, time) {
    //Открытие модалки поистечении заданного времени
    setTimeout(function () {
      var display;
      ...
      if (!display) {
        ...
        document.body.style.overflow = "hidden";
        var scroll = calcScroll(); //Вычисленная ширина скролла
  ...
  function calcScroll() {
    ...
    var scrollWidth = div.offsetWidth - div.clientWidth; //Удаляем созданный элемент
  ...
  openByScroll(".fixed-gift"); //Попап при отсутствии действий на странице
...
var pictureSize = function pictureSize(imgSelector) {
  var blocks = document.querySelectorAll(imgSelector); //Вместо заглушки показываем нужные картинки

  var showImg = function showImg(block) {
    var img = block.querySelector("img"); //Изменяем путь к картинке
  ...
  links.forEach(function (link) {
    link.addEventListener("click", function (event) {
      ...
      function step(time) {
        ...
        var progress = time - start; //Кол-во пикселей, которое нужно пролистать в течение этой анимации
  ...
  function createCards(response) {
    response.forEach(function (_ref) {
      var src = _ref.src,
          title = _ref.title,
          link = _ref.link;
      var card = document.createElement("div"); //Стилизация и анимация

      card.classList.add("animated", "fadeInUp", "col-sm-3", "col-sm-offset-0", "col-xs-10", "col-xs-offset-1"); //Верстка самой карты
...
var sliders = function sliders(slides, dir, prev, next) {
  ...
  var items = document.querySelectorAll(slides); //Слайды
  ...
  showSlides(slideIndex); //Изменение (+/-) активного слайда
  ...
  activateAnimation(); //Если мышь внутри слайдера
